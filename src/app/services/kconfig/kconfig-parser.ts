...
import { ConfigType, IConfig, IKConfigParser, KConfig } from './KConfigParser.types';
...
class Config extends KConfigItem implements IConfig {
  ...
  private _type: ConfigType = ConfigType.String;
  ...
  constructor(parser: KConfigParser, owner: KConfigItem, tokens: string[]) {
    ...
    for (let i = 1; i < tokens.length; i++) {
      ...
      if (Object.values(ConfigType).includes(token as ConfigType)) {
        this._type = token as ConfigType;
...
export class KConfigParser implements IKConfigParser {
